...
int main()
{
    ...
    int i = 0, k = 0, m = 0, n = 0, number_of_circles, number_of_triangles,
        number_of_figures, error = 1;
    ...
    for (int i = 0; i < number_of_triangles; i++) {
        ...
        int x = Circle[0].rad;
        printf("intersects:\n\t");
        if ((x >= Triangle[i].first.x && x >= Triangle[i].first.y)
            || (x >= Triangle[i].second.x && x >= Triangle[i].second.y)
            || (x >= Triangle[i].third.x && x >= Triangle[i].third.y)) {
            printf("%d. circle\n\n", 1);
        }
    }
